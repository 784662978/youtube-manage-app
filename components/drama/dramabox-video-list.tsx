...
export function DramaboxVideoList({ languages, onNotification }: DramaboxVideoListProps) {
  const [data, setData] = React.useState<DramaboxVideo[]>([])
  ...
  // 筛选条件（UI 绑定，不会自动触发查询）
  const [exclusiveType, setExclusiveType] = React.useState("独家")
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      if (searchParams.exclusiveType) params.exclusive_type = searchParams.exclusiveType
      ...
      const result = await apiClient.get<ApiResponse<PageResponse<DramaboxVideo>>>(
        "/dramabox/videos",
        params
      )
      ...
      console.error("Failed to fetch dramabox videos:", error)
  ...
  const handlePullData = async () => {
    ...
    try {
      await apiClient.post<ApiResponse<boolean>>(
        "/dataPull/dramabox-latest",
        {}
      )
      onNotification("拉取数据成功", "success")
      fetchData()
    } catch (error) {
      const message = error instanceof Error ? error.message : "拉取数据失败"
  ...
  // 独家类型 badge 样式
  const exclusiveBadge: Record<string, { variant: "default" | "secondary" | "outline" }> = {
    "独家": { variant: "default" },
    "非独家": { variant: "secondary" },
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Select value={exclusiveType || "__all__"} onValueChange={(v) => setExclusiveType(v === "__all__" ? "" : v)}>
        ...
        <Select value={language || "__all__"} onValueChange={(v) => setLanguage(v === "__all__" ? "" : v)}>
          ...
          <SelectContent>
            ...
            {languages.map((lang) => (
              <SelectItem key={lang.zh_name} value={lang.zh_name}>
                {lang.display_name || lang.zh_name}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
